  ...
  <div class="w-screen relative left-1/2 right-1/2 -ml-[50vw] -mr-[50vw]"
    ...
    <div class="max-w-7xl mx-auto px-4 py-8">
      ...
      <header class="rounded-[28px] bg-gradient-to-r from-blue-800 to-blue-600 text-white p-8 md:p-10 shadow-soft">
        <h1 class="text-3xl md:text-4xl font-black !text-white">Aprende con nosotros</h1>
        <p class="mt-2 text-white/90 max-w-2xl">Tutoriales para comprar, gestionar tu cuenta y aprovechar la tienda.</p>

        <form method="get" class="mt-6 max-w-xl">
          <div class="bg-white/95 rounded-full p-2 flex items-center gap-2 shadow">
            <input name="q" value="<?php echo esc_attr($search_q); ?>"
              class="w-full px-4 py-2 rounded-full outline-none text-blue-900"
              placeholder="Buscar tutorial..." />
            <button class="px-5 py-2 rounded-full bg-yellow-400 text-blue-900 font-extrabold">Buscar</button>
          </div>
        </form>
      </header>

      <!-- Texto editable desde WP (tú lo editas en la página Aprende) -->
      ...
      <section class="mt-8">
            ...
            ?>
              <article class="bg-white rounded-[28px] border border-azulOsc/10 shadow-soft overflow-hidden hover:shadow-xl hover:-translate-y-1 transition">
                <a href="<?php the_permalink(); ?>" class="block">
                  ...
                  <div class="p-5">
                    <h3 class="text-azulOsc font-black"><?php the_title(); ?></h3>
                    <p class="mt-2 text-sm text-azulOsc/70"><?php echo esc_html(wp_trim_words(get_the_excerpt(), 18)); ?></p>
                    <div class="mt-4 inline-flex px-4 py-2 rounded-2xl bg-amarillo text-azulOsc font-extrabold">Ver tutorial →</div>
                  </div>
                </a>
              </article>
        ...
        <?php else: ?>
          <div class="mt-6 bg-white rounded-[28px] border border-azulOsc/10 shadow-soft p-10 text-center">
            <h3 class="text-azulOsc font-black text-lg">Aún no hay tutoriales</h3>
            <p class="text-azulOsc/70 mt-2">Crea entradas en la categoría “aprende”.</p>
          </div>
        <?php endif; ?>
      </section>
