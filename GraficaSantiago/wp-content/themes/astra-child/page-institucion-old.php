...
get_header();
echo '<div style="padding:20px;background:#00ff0033;border:1px solid #0a0;">TEMPLATE INSTITUCION CARGADO ✅</div>';


// 1) Obtener institución por query param
$inst_id = isset($_GET['inst']) ? absint($_GET['inst']) : 0;

// Alternativa por slug: /institucion/?slug=unidad-educativa-san-andres
if (!$inst_id && !empty($_GET['slug'])) {
    $slug = sanitize_title(wp_unslash($_GET['slug']));
    $p = get_page_by_path($slug, OBJECT, 'gsi_institucion');
    if ($p) $inst_id = (int) $p->ID;
}
