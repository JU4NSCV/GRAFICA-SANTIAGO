...
if (!function_exists('gsi_badge_for_post')) {
    function gsi_badge_for_post()
    {
        ...
        // Ajusta slugs a tus categorías reales
        ...
        if (in_array($slug, ['historia', '40-anos'], true))   return ['HISTORIA', 'bg-white/90 text-azulOsc'];
...
// Imágenes demo (puedes cambiarlas por tuyas)
...
$banner3 = "https://static.vecteezy.com/system/resources/thumbnails/004/299/835/small/online-shopping-on-phone-buy-sell-business-digital-web-banner-application-money-advertising-payment-ecommerce-illustration-search-free-vector.jpg";
...
$page_raw  = get_query_var('page'); // en páginas a veces viene aquí
$paged     = max(1, (int)($paged_raw ?: $page_raw ?: (isset($_GET['paged']) ? (int)$_GET['paged'] : 1)));
...
// URLs para chips dentro de ESTA misma página
function gsi_filter_url($base_url, $cat_slug, $search_q, $paged = null)
{
    $params = [];
    if (!empty($cat_slug)) $params['cat'] = $cat_slug;
    if (!empty($search_q)) $params['q']   = $search_q;
    if (!empty($paged))    $params['paged'] = $paged;
    return add_query_arg($params, $base_url);
}
$aprende_page   = get_page_by_path('aprende');
$novedades_page = get_page_by_path('novedades');
$historia_page  = get_page_by_path('historia');

$aprende_url   = $aprende_page ? get_permalink($aprende_page) : gsi_filter_url($base_url, 'aprende', $search_q);
$novedades_url = $novedades_page ? get_permalink($novedades_page) : gsi_filter_url($base_url, 'novedades', $search_q);
$historia_url  = $historia_page ? get_permalink($historia_page) : gsi_filter_url($base_url, 'historia', $search_q);
        ...
        <div class="max-w-7xl mx-auto px-4 pt-6 pb-10">
            ...
            <section class="relative">
                <div class="relative rounded-[28px] overflow-hidden shadow-soft border border-azulOsc/10 bg-white">
                    <div class="relative aspect-[16/8] min-h-[240px] sm:min-h-[340px] md:min-h-[440px] lg:min-h-[520px]" id="heroSlider">

                        <?php
                        $slides = [
                            [
                                'img' => $banner1,
                                'tag' => ['MEGA SANTIAGO', 'bg-amarillo text-azulOsc'],
                                'title' => 'Ideas, tips y novedades',
                                'desc' => 'Contenido útil para escuela, oficina y creatividad — con productos que sí encuentras aquí.',
                                'btn1' => ['#posts', 'Ver publicaciones →', 'bg-amarillo text-azulOsc'],
                                'btn2' => [$shop_url, 'Ir a catálogo', 'bg-white/90 text-azulOsc hover:bg-white'],
                            ],
                            [
                                'img' => $banner2,
                                'tag' => ['APRENDE', 'bg-white/90 text-azulOsc'],
                                'title' => 'Tutoriales y organización',
                                'desc' => 'Lettering, técnicas, hábitos y productividad con materiales de papelería.',
                                'btn1' => [$aprende_url, 'Ver Aprende →', 'bg-amarillo text-azulOsc'],
                                'btn2' => ['', '', ''],
                                'href' => $aprende_url,
                            ],
                            [
                                'img' => $banner3,
                                'tag' => ['NOVEDADES', 'bg-amarillo text-azulOsc'],
                                'title' => 'Eventos y promociones',
                                'desc' => 'Cronograma, descuentos, campañas y fechas importantes.',
                                'btn1' => [$novedades_url, 'Ver Novedades →', 'bg-white/90 text-azulOsc hover:bg-white'],
                                'btn2' => ['', '', ''],
                                'href' => $novedades_url,
                            ],
                        ];
                        ...
                        <?php foreach ($slides as $i => $s): ?>
                            <div class="absolute inset-0 transition-opacity duration-700 <?php echo $i === 0 ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'; ?>"
                                data-slide
                                data-href="<?php echo !empty($s['href']) ? esc_url($s['href']) : ''; ?>">

                                <img src="<?php echo esc_url($s['img']); ?>" class="w-full h-full object-cover" alt="Slide">
                                <div class="absolute inset-0"
                                    style="background: linear-gradient(90deg, rgba(0,0,0,.70) 0%, rgba(0,0,0,.25) 55%, rgba(0,0,0,0) 100%);"></div>
                                ...
                                <div class="absolute inset-0 flex items-center">
                                    <div class="p-6 sm:p-10 md:p-14 max-w-2xl text-white">
                                        ...
                                        <h1 class="mt-3 text-2xl sm:text-4xl md:text-5xl font-black leading-tight !text-white drop-shadow">
    ...
    <section class="max-w-7xl mx-auto px-4 mt-10" id="apartados">
        <div class="flex items-end justify-between gap-4 mb-6">
            <div>
                ...
                <p class="text-sm text-azulOsc/70 mt-1">Aprende, novedades y contenido que te ahorra tiempo.</p>
            </div>
            <a href="#posts"
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <a href="<?php echo esc_url($historia_url); ?>"
                class="lg:col-span-7 group relative overflow-hidden rounded-[28px] bg-white border border-azulOsc/10 shadow-soft hover:shadow-xl hover:-translate-y-1 transition">
                <div class="relative min-h-[340px] md:min-h-[440px]">
                    <img src="<?php echo esc_url($img40); ?>" alt="Historia"
                    ...
                    <div class="absolute bottom-0 left-0 right-0 p-6 text-white">
                        <h3 class="text-xl md:text-2xl font-black !text-white drop-shadow">Nuestra trayectoria</h3>
                        <p class="mt-1 text-sm text-white/85 line-clamp-2 max-w-xl">
                            Conoce cómo Mega Santiago ha crecido con Loja y con tu día a día.
                        </p>
                        <div class="mt-4 inline-flex items-center gap-2 px-4 py-2 rounded-2xl bg-white/90 text-azulOsc font-extrabold group-hover:bg-amarillo transition shadow">
                            Entrar →
                        </div>
                    </div>
                </div>
            ...
            <div class="lg:col-span-5 grid grid-rows-2 gap-6">
                ...
                <a href="<?php echo esc_url($aprende_url); ?>"
                    ...
                    <div class="relative min-h-[210px] md:min-h-[220px]">
                        <img src="<?php echo esc_url($imgAprende); ?>" alt="Aprende"
                            class="absolute inset-0 w-full h-full object-cover group-hover:scale-[1.04] transition duration-700">
                        <div class="absolute inset-0 bg-gradient-to-t from-black/75 via-black/10 to-transparent"></div>
                        ...
                        <div class="absolute bottom-0 left-0 right-0 p-5 text-white">
                            <h3 class="text-xl md:text-2xl font-black !text-white drop-shadow">Tips y tutoriales</h3>
                            <p class="mt-1 text-xs text-white/85 line-clamp-2">
                                Organización, lettering y productividad con papelería.
                            </p>
                ...
                <a href="<?php echo esc_url($novedades_url); ?>"
                    class="group relative overflow-hidden rounded-[28px] bg-white border border-azulOsc/10 shadow-soft hover:shadow-xl hover:-translate-y-1 transition">
                    <div class="relative min-h-[210px] md:min-h-[220px]">
                        <img src="<?php echo esc_url($imgCrono); ?>" alt="Novedades"
                            class="absolute inset-0 w-full h-full object-cover group-hover:scale-[1.04] transition duration-700">
                        <div class="absolute inset-0 bg-gradient-to-t from-black/75 via-black/10 to-transparent"></div>
                        ...
                        <div class="absolute bottom-0 left-0 right-0 p-5 text-white">
                            <h3 class="text-xl md:text-2xl font-black !text-white drop-shadow">Eventos y promociones</h3>
                            <p class="mt-1 text-xs text-white/85 line-clamp-2">
                                Cronograma, campañas y fechas importantes.
                            </p>
                            <div class="mt-3 inline-flex items-center gap-2 text-xs font-extrabold text-white/95">
                                Entrar <span aria-hidden="true">→</span>
                            </div>
    ...
    <section class="max-w-7xl mx-auto px-4 mt-14" id="posts">
        ...
        <div class="flex flex-wrap gap-2 mb-8">
            ...
            foreach ($chips as $chip):
                ...
                $chip_url = gsi_filter_url($base_url, $chip[1], $search_q);
            ?>
        ...
        <?php if ($q->have_posts()): ?>
            ...
            <?php
            // Paginación por querystring (segura en páginas)
        ...
        <!-- CTA final -->
        <div class="mt-14 rounded-[28px] p-8 md:p-10 text-white shadow-soft relative overflow-hidden"
            style="background: linear-gradient(135deg, var(--gs-azul-osc) 0%, #223044 100%);">
            ...
            <div class="relative flex flex-col md:flex-row items-center justify-between gap-6">
                <div>
                    <h3 class="text-2xl font-black text-white">¿Listo para comprar más rápido?</h3>
        ...
        (function() {
            ...
            function render() {
                ...
                slides.forEach((s, i) => {
                    s.classList.toggle('opacity-100', i === idx);
                    s.classList.toggle('opacity-0', i !== idx);
                    s.classList.toggle('pointer-events-auto', i === idx);
            ...
            function go(i) {
                ...
                root.addEventListener('click', (e) => {
                    if (e.target.closest('[data-prev],[data-next],[data-dots],a,button,input,select,textarea')) return;
